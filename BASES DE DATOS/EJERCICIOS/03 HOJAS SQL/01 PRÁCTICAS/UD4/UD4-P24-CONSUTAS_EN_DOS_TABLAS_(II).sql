...
--A)
SELECT APENOM FROM ALUMNOS A, ASIGNATURAS ASIG, NOTAS N
WHERE A.DNI = N.DNI AND ASIG.COD = N.COD
    AND N.NOTA BETWEEN 7 AND 8
    AND ASIG.NOMBRE = 'FOL';
    
--B)
SELECT DISTINCT NOMBRE FROM ASIGNATURAS A, NOTAS N
WHERE A.COD = N.COD
    AND A.COD NOT IN(SELECT COD FROM NOTAS
                     WHERE NOTA < 5);